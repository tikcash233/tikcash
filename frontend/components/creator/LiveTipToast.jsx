...
export default function LiveTipToast({ tip, onClose, duration = 8000 }) {
  ...
  // Prefer server-provided creator_amount (net to creator). If missing, compute net after platform_net (18%) and paystack fee (2%).
  const round2 = (v) => Math.round((Number(v) + Number.EPSILON) * 100) / 100;
  const rawAmt = Number(tip.amount || 0);
  const amount = Number(tip.creator_amount != null ? tip.creator_amount : round2(rawAmt - (rawAmt * 0.18) - (rawAmt * 0.02)));
