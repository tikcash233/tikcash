...
export default function CreatorDashboard() {
  ...
  const applyTipToCreator = (tip) => {
    ...
    // compute net as amount minus platform_net (18%) and paystack fee (2%) and round to 2 decimals.
    ...
    if (tip.creator_amount != null) {
      ...
      amt = round2(raw - (raw * 0.18) - (raw * 0.02));
  ...
  const raw = Number(tip.amount || 0) || 0;
  const net = Number(tip.creator_amount != null ? tip.creator_amount : round2(raw - (raw * 0.18) - (raw * 0.02)));
    const notification = {
      id: Date.now(),
      amount: net,
  ...
  useEffect(() => {
    ...
    // Always prefer explicit backend origin for SSE (proxy layers may buffer event streams)
    const API_BASE = (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_API_URL)
      ? import.meta.env.VITE_API_URL
      : ((typeof window !== 'undefined' && window.__API_BASE__) || '');
    // If no explicit base is set, we still attempt relative path (works in local dev proxy),
    // but in production set VITE_API_URL (Render static site env) so SSE connections go direct.
